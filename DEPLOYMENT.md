...
If the build log says `/vercel/path0/package.json` is missing, the Vercel project is pointing at the wrong repository, branch, or Root Directory. Select the GitHub repository containing this root-level `package.json`, set Root Directory to `.` (or blank), and redeploy. `vercel.json` pins the build, install, and output settings but cannot override an incorrect Vercel Root Directory.
